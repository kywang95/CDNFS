...
# simulation <- "realdata_classification_"
# evaluation <- "ACC"
# cases <- "ORL"
# method <- c("CDNFS","FOCI", "DC_SIS", "DCOL", "mRMR", "NMIFS")

simulation <- "realdata_regression_"
evaluation <- "MSE"
cases <- "slice_localization_data"
method <- c("CDNFS", "DC_SIS", "mRMR", "NMIFS")
...
for (iter in 1:length(cases)){
  for (i_repeat in start_times:repeat_times){
    ...
    for (i_fold in 1:k){
      for (imethod in 1:length(method)){
        data <- read.csv(paste0(pathname, "result_", simulation, method[imethod], 
                                "_", cases[iter], "_", i_fold, ".csv"))
  ...
  write.csv(outsample_result_svm, paste0("../", evaluation, "_outsample_SVM_", cases[iter], ".csv"))
  write.csv(outsample_result_CART, paste0("../", evaluation, "_outsample_CART_", cases[iter], ".csv"))
  write.csv(outsample_result_lightGBM, paste0("../", evaluation, "_outsample_lightGBM_", cases[iter], ".csv"))
  write.csv(outsample_result_NN2, paste0("../", evaluation, "_outsample_MLP_", cases[iter], ".csv"))
